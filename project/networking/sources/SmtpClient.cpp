...
bool SmtpClient::Login()
{
	...
	while (true)
	{
		...
		if (strcontains(resp.c_str(), "334"))
		{
			...
			CoreLib::Base64 codec_b64;
			std::string decoded_str;
			unsigned long olen;
			std::vector<unsigned char> decoded_buffer = codec_b64.Decocde(tokens[1]);
			decoded_str = (char*)decoded_buffer.data();

			std::vector<unsigned char> encoded_buffer;
			std::vector<unsigned char> plain_buffer;

			if (strcontains(decoded_str.c_str(), "Username"))
			{
				plain_buffer.clear();
				std::copy(username.begin(), username.end(), std::back_inserter(plain_buffer));

				std::string encoded_uname = codec_b64.Encode(plain_buffer);
			...
			{
				plain_buffer.clear();
				std::copy(password.begin(), password.end(), std::back_inserter(plain_buffer));

				std::string encoded_password = codec_b64.Encode(plain_buffer);
